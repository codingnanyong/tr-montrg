...
{
    public partial class FrameCtrlEdit
    {
        #region Fields
        ...
        public FrameCtrl FrameCtrl { get; set; } = new FrameCtrl();
        ...
        protected async Task HandleValidSubmit()
        {
            ...
            if (IsAddMode)
            {
                var itemCreated = await FrameCtrlDataService.Create(FrameCtrl);
                if (itemCreated != null)
                {
            ...
            {
                var success = await FrameCtrlDataService.Update(FrameCtrl);
                if (success)
                {
                    StatusClass = "alert-success";
        ...
        {
            var success = await FrameCtrlDataService.Delete(FrameCtrl.DeviceId);
            if (success)
            {
                StatusClass = "alert-success";
        ...
        {
            NavManager.NavigateTo($"/admin/framectrls/{PlantId}/{DeviceId}");
        }
    }
}
